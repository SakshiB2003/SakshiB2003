# 👋 Hi, I'm Sakshi!

I'm a **creative frontend enthusiast** who loves building **interactive and visually engaging web apps**.  
I blend **design with function** — and add a touch of ✨personality✨ to every project.

---

## 🌟 Featured Projects

### 1. **MUSE-Journal** – Blogging Web App
A simple blogging app built with **Node.js, Express.js, and EJS**. Users can create, view, edit, and delete posts. Perfect for learning server-side rendering and CRUD operations.

**Key Features:**  
- 📝 Create, view, edit & delete posts  
- ⚡ Dynamic routing with Express.js  
- 🎨 Responsive design with custom CSS  

**Tech Stack:**  
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white)  
![Express](https://img.shields.io/badge/Express-000000?style=flat-square&logo=express&logoColor=white)  
![EJS](https://img.shields.io/badge/EJS-000000?style=flat-square)  

**🔗 Links:**  
[![Live Demo](https://img.shields.io/badge/Live%20Demo-007ACC?style=for-the-badge&logo=google-chrome&logoColor=white)](https://muse-journal.onrender.com/)  
[![View Code](https://img.shields.io/badge/View%20Code-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/SakshiB2003/Muse-Journal)

---

### 2. **Amazon Homepage Clone** – Frontend UI
Fully responsive **Amazon homepage clone** built with HTML & CSS. Demonstrates layout and responsive design skills.

**Key Features:**  
- 📱 Responsive navbar & hero banner  
- 🖼️ Product panels with hover effects  

**Tech Stack:**  
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)  
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)  

**🔗 Links:**  
[![Live Demo](https://img.shields.io/badge/Live%20Demo-007ACC?style=for-the-badge&logo=google-chrome&logoColor=white)](https://amazon-clone-gold-zeta.vercel.app/)  
[![View Code](https://img.shields.io/badge/View%20Code-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/SakshiB2003/amazon-clone)

---

### 3. **Midnight To-Do List** – Interactive Task Manager
A dark-themed task manager with animations, motivational quotes, and sound feedback.

**Key Features:**  
- ✅ Add, complete, delete tasks  
- 🌈 Rotating motivational quotes  
- 🔔 Sound feedback & toast notifications  

**Tech Stack:**  
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)  
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)  
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)  

**🔗 Links:**  
[![Live Demo](https://img.shields.io/badge/Live%20Demo-007ACC?style=for-the-badge&logo=google-chrome&logoColor=white)](https://sakshib2003.github.io/midnight-todo-list/)  
[![View Code](https://img.shields.io/badge/View%20Code-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/SakshiB2003/midnight-todo-list)

---

### 4. **BTS Photocard Collection** – Fan Project
Flip, like, and organize BTS photocards.

**Key Features:**  
- 🔄 Flip cards front/back  
- 💖 Like favorite cards  
- 🎚️ Filter & sort by member  

**Tech Stack:**  
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)  
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)  
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)  

**🔗 Links:**  
[![Live Demo](https://img.shields.io/badge/Live%20Demo-007ACC?style=for-the-badge&logo=google-chrome&logoColor=white)](https://bts-photocard.vercel.app/)  
[![View Code](https://img.shields.io/badge/View%20Code-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/SakshiB2003/bts-photocard)

---

### 5. **Palmer Penguins Data Analysis (R)** – Data Science
Analyze **Palmer Penguins dataset** using R with visualizations and summary stats.

**Key Features:**  
- ✅ Data cleaning & transformation  
- 📊 Visualizations with ggplot2  
- 📈 Species comparison  

**Tech Stack:**  
![R](https://img.shields.io/badge/R-276DC3?style=flat-square&logo=r&logoColor=white)  

**🔗 Link:**  
[![View Full Project](https://img.shields.io/badge/View%20Code-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/SakshiB2003/palmer-penguins-project)

---

## 💼 Other Projects
- **Jimin — MUSE Fan Page** – Interactive album showcase with sliders & track highlights [![Live Demo](https://img.shields.io/badge/Live%20Demo-007ACC?style=for-the-badge&logo=google-chrome&logoColor=white)](https://jimin-muse.vercel.app/)  
- **Drumkit, Pinterest Clone, Card Slider, etc.** – Small interactive JS & frontend demos  

---

## ✨ Tech Stack
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)  
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)  
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)  
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white)  
![Express](https://img.shields.io/badge/Express-000000?style=flat-square&logo=express&logoColor=white)  
![EJS](https://img.shields.io/badge/EJS-000000?style=flat-square)  
![R](https://img.shields.io/badge/R-276DC3?style=flat-square&logo=r&logoColor=white)  

---

## ✉️ Let's Connect!
- 📫 LinkedIn: [https://www.linkedin.com/in/sakshi-bajpai-4b5736230](https://www.linkedin.com/in/sakshi-bajpai-4b5736230)  
- 📷 GitHub Portfolio: [https://github.com/SakshiB2003](https://github.com/SakshiB2003)  

> “It’s not just about building — it’s about creating things people love to use.”  
> — Sakshi Bajpai
